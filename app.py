...
def post(post_id):
    ...
    if post is None:
      app.logger.info(f"Article with id {post_id} does not exists")
      return render_template('404.html'), 404
    else:
      app.logger.info(f"Article {post['title']} retrieve")
      return render_template('post.html', post=post)

# Define the About Us page
@app.route('/about')
def about():
    app.logger.info("The page About was render")
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            app.logger.info(f"The Article with title {title} was create successfully")
